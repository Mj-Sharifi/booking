...
const validateToken = async (request: NextRequest) => {
  ...
  response.cookies.set("test1","test",{path:"/"})
  return response
  // const user_info = request.cookies.get('user_info');
  // const locale = getLocale(request);
  // const { pathname } = request.nextUrl;
  ...
  // const response = NextResponse.next();
  // response.cookies.set("test1","test1",{path:"/"})
  // if (user_info) {
  //   try {
  //     const { jwt } = JSON.parse(user_info.value);
  //     // Validate token via API call
  ...
  //     return handleI18nRouting(request); // Return here if the token is valid
  //   } catch (error) {
...
//     ) // Pass the response object here to modify it if necessary
//   );
// }
const test = ()=>{
  const response = NextResponse.next()
